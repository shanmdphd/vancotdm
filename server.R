...
library(rsconnect)
# deployApp()
# setAccountInfo(name='catholic', token='35A4CF5EFA0831AA195176F3F0C2A40D', secret='Onpx1AJJQ5HXlxqdTzQ/O1jODgLF0075KzGV9flR')
...
shinyServer(function(input, output) {
  ...
  output$dateText  <- renderText({
    ...
    if (input$sex=='Male')
    {
      crcl=( (140-input$age) * (input$weight) )  / (72*input$scr)
    }
    
    if (input$sex=="Female")
    {
      crcl=( (140-input$age) * (input$weight) *0.85 )  / (72*input$scr)
  ...
  output$outputtable <- renderTable({
    ...
    if (input$Observations=='1')
    {
      ...
      prtx=prt2[2,c("pointtime","observedConc","predictedConc")]
    ...
    if (input$Observations=='2')
    {
      ...
      prtx=prt2[2,c("pointtime1","observedConc1","predictedConc1")]
    }
    prtx
  })
